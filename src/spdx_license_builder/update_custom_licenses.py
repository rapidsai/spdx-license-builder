...
class NvidiaLicenseHTMLParser(html.parser.HTMLParser):
    ...
    def handle_data(self, data):
        ...
        if not self.capturing:
            ...
            elif self.found_title and text == "NVIDIA Software License Agreement":
                self.capturing = True
...
def fetch_license_from_url(url: str, license_id: str) -> str | None:
    """
    ...
    try:
        ...
        parser = NvidiaLicenseHTMLParser() if "nvidia.com" in url.lower() else LicenseHTMLParser()
        parser.feed(html_content)
        license_text = parser.get_text()

        if not license_text or len(license_text) < 100:
            print(
                f"Warning: Fetched content seems too short ({len(license_text)} chars)",
                file=sys.stderr,
            )
            return None
...
def update_custom_licenses(base_path: Path | None = None) -> dict[str, bool]:
    """
