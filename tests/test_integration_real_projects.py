...
class TestRealCCCLStructure:
    ...
    def test_cccl_deduplication(self, cccl_structure):
        ...
        assert any('cccl/cpp/LICENSE' in p and 'cub' not in p and 'thrust' not in p and 'libcudacxx' not in p 
                   for p in remaining_paths), f"Root not found in paths: {remaining_paths}"
        ...
        if len(cccl_paths) > 1:
            ...
            component_count = sum(1 for p in cccl_paths 
                                if '/cub/' in p or '/thrust/' in p or '/libcudacxx/' in p)
            root_count = sum(1 for p in cccl_paths 
                           if '/cpp/LICENSE' in p and 'cub' not in p and 'thrust' not in p and 'libcudacxx' not in p)
...
class TestRealRAPIDSProjects:
    ...
    def test_rapids_deduplication(self, rapids_structure):
        ...
        rapids_dirs = [rapids_structure[p] for p in ['raft', 'cudf', 'cuco'] 
                      if p in rapids_structure]
        ...
        rapids_licenses_before = sum(
            1 for info in content_map.values()
            if any('raft' in p or 'cudf' in p or 'cuco' in p 
                   for p in info['paths'].keys())
        ...
        rapids_licenses_after = sum(
            1 for info in deduplicated.values()
            if any('raft' in p or 'cudf' in p or 'cuco' in p 
                   for p in info['paths'].keys())
            and 'Apache' in info['content']
        )
...
class TestCombinedRealProjects:
    ...
    def test_full_deduplication_pipeline(self, combined_structure):
        ...
        cccl_paths = []
        for info in deduplicated.values():
            cccl_paths.extend([p for p in info['paths'].keys() if 'cccl' in p])
        
        if cccl_paths:
            # Should have root
            has_root = any('/LICENSE' in p and 'cub' not in p and 'thrust' not in p 
                          and 'libcudacxx' not in p for p in cccl_paths)
            assert has_root
            
            # Should not have components
            has_components = any('cub/LICENSE' in p or 'thrust/LICENSE' in p 
                               or 'libcudacxx/LICENSE' in p for p in cccl_paths)
    ...
    def test_realistic_output_size(self, combined_structure):
        ...
        reduction_percent = (1 - len(with_dedup) / len(without_dedup)) * 100 if len(without_dedup) > 0 else 0
